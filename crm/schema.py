...
class BulkCreateCustomers(graphene.Mutation):
    """Mutation for bulk customer creation with partial success."""
    ...
    def mutate(cls, root, info, customers):
        ...
        for index, c in enumerate(customers):
            ...
            try:
                ...
                errors.append(FieldError(field=f"customers[{index}]", message=f"Error creating record: {str(e)}"))
...
class CreateOrder(graphene.Mutation):
    """Mutation to create an order with nested product associations."""
    ...
    def mutate(cls, root, info, customer_id, product_ids, order_date=None):
        ...
        if not product_ids:
            ...
            if len(valid_products) != len(product_ids):
                ...
                errors.append(FieldError(field="product_ids", message=f"Invalid product ID(s): {', '.join(map(str, invalid_ids))}."))
